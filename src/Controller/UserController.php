    ...
    public function edit(Request $request, User $user, EntityManagerInterface $entityManager, UserPasswordHasherInterface $passwordHasher, UserLogger $userLogger): Response
{
    ...
    // For handling 2FA authentication (you would set/get the authCode here)
    #[Route('/{id}/enable-2fa', name: 'app_user_enable_2fa', methods: ['POST'])]
    public function enableTwoFactorAuth(Request $request, User $user, EntityManagerInterface $entityManager): Response
    {
        // Generate a new authCode and set it to the user
        $authCode = random_int(100000, 999999); // Example: Random 6-digit code
        $user->setEmailAuthCode((string)$authCode); // Set authCode manually

        // You can also send the authCode to the user via email or SMS here
